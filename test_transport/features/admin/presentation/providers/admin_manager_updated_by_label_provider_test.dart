...
import 'package:local_transport/features/admin/presentation/providers/admin_manager_updated_by_label_provider.dart';

void main() {
  group('adminManagerUpdatedByLabelProvider', () {
    test('devolve email original quando o valor já é email', () async {
      ...
      final result = await container.read(
        adminManagerUpdatedByLabelProvider('gestor@example.com').future,
      );
    ...
    test('resolve uid antigo para email', () async {
      ...
      final result = await container.read(
        adminManagerUpdatedByLabelProvider('manager_1').future,
      );
  ...
  Future<Map<String, AuditSubjectIdentity>> resolveSubjectIdentities(
    ...
    return Map<String, AuditSubjectIdentity>.fromEntries(
      userIds
          ...
          .map((userId) => MapEntry(userId, identities[userId]!)),
    );
  }

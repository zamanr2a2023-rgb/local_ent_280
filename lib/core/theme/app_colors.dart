import 'package:flutter/material.dart';

/// Design tokens from app_details/details.md
abstract final class AppColors {
  static const Color background = Color(0xFFF7F9FB);
  static const Color primary = Color(0xFF001736);
  static const Color primaryContainer = Color(0xFF002B5B);
  static const Color primaryFixedDim = Color(0xFFA9C7FF);
  static const Color onPrimary = Color(0xFFFFFFFF);
  static const Color secondary = Color(0xFF0051D5);
  static const Color onSecondary = Color(0xFFFFFFFF);
  /// Primary accent from home mockup: buttons, icons, active nav.
  static const Color accent = Color(0xFF2B6EFF);
  static const Color onAccent = Color(0xFFFFFFFF);
  /// Light blue circle behind quick-action icons.
  static const Color accentSurface = Color(0xFFE6F0FF);
  static const Color secondaryContainer = Color(0xFF316BF3);
  static const Color onSecondaryContainer = Color(0xFFFEFCFF);
  static const Color secondaryFixed = Color(0xFFDBE1FF);
  static const Color onSecondaryFixed = Color(0xFF00174B);
  static const Color tertiary = Color(0xFF07182B);
  static const Color tertiaryContainer = Color(0xFF1D2D41);
  static const Color onTertiary = Color(0xFFFFFFFF);
  static const Color onTertiaryContainer = Color(0xFF8495AD);
  static const Color onBackground = Color(0xFF191C1E);
  static const Color onSurface = Color(0xFF191C1E);
  static const Color onSurfaceVariant = Color(0xFF43474F);
  /// Muted labels (e.g. Saldo Disponível, action tile captions).
  static const Color labelMuted = Color(0xFF6B7280);
  /// Input field fill on home sheet.
  static const Color inputFill = Color(0xFFF3F4F6);
  static const Color surfaceContainerHigh = Color(0xFFE6E8EA);
  static const Color surfaceContainer = Color(0xFFECEEF0);
  static const Color surfaceContainerLow = Color(0xFFF2F4F6);
  static const Color surfaceContainerLowest = Color(0xFFFFFFFF);
  static const Color outline = Color(0xFF747780);
  static const Color outlineVariant = Color(0xFFC4C6D0);
  static const Color surfaceVariant = Color(0xFFE0E3E5);
  static const Color onPrimaryContainer = Color(0xFF7594CA);
  static const Color primaryFixed = Color(0xFFD6E3FF);
  static const Color error = Color(0xFFBA1A1A);
  static const Color onError = Color(0xFFFFFFFF);
  static const Color errorContainer = Color(0xFFFFDAD6);
  static const Color onErrorContainer = Color(0xFF93000A);
}
